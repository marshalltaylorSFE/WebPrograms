...
void ESPWebServer::sendFile( File * file, const char* content_type ) {
    String header;
	long int filePtr = 0;
	int fileSize = file->size();
	uint8_t data[1000];
	int i;
	_prepareHeader(header, 200, content_type, fileSize);
	...
	{
		for(i = 0; (i < 1000)&&(file->available()); i++)
		{
			data[i] = file->read();
		}
//		Serial.println(str.c_str());
//		Serial.println("****");
		_currentClient.write(data, i);
    }	
}
