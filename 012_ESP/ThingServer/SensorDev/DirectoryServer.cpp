...
int FileServer::startServer( void ) {
	...
	for(int i = 0; i < files.size(); i++)
	{
		...
		Serial.print(files[i].path.c_str());
		Serial.println();
		serveStatic(files[i].HTMLpath.c_str(), SD, files[i].path.c_str(), "public"); //"no-cache, no-store, must-revalidate"
	}	
